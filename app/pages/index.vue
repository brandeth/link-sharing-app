<script setup lang="ts">
useHead({ title: 'Link Sharing App' })
</script>

<template>
  <main class="mx-auto flex min-h-screen max-w-5xl flex-col items-start justify-center gap-4 px-6">
    <h1 class="text-preset-1 text-fg-heading">Link Sharing App</h1>
    <p class="text-preset-3 text-fg-secondary">Nothing here yet.</p>
    <NuxtLink to="/links" class="font-semibold text-brand underline underline-offset-4">
      Dashboard
    </NuxtLink>
    <NuxtLink to="/design-system" class="font-semibold text-brand underline underline-offset-4">
      Design system
    </NuxtLink>
  </main>
</template>
